...
<details markdown="1">
<summary class='jb-small' style="color:blue">generate rsa key</summary>
<hr class='division3'>
`client`
```dos
...
`server` : After <u>copy</u> content of <b>id_rsa.pub</b> on windows, <u>paste</u> it into <b>~/.ssh/authorized_keys</b> on server(ex, linux)<br>
~/.ssh/authorized_keys
```bash
$ vim authorized_keys
```
