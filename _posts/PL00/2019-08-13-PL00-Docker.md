...
$ docker run -itd --restart=always --name "[image_name]" [image] [command]
...
$ docker search [image]
$ docker pull [image]
$ docker run -itd --name [image_name(origin)] [image]
$ docker attach [image_name(origin)]
```
<br>
...
```bash
$ docker commit [image_name(origin)] [image_name(new)]
```
step3] Remove origin image file
```bash
$ docker stop [image_name(origin)];docker rm ubt
```
step4] create new
